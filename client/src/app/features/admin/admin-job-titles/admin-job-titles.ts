...
export class AdminJobTitlesComponent implements OnInit {
  ...
  formData = signal<{ title: string }>({ title: '' });
  ...
  loadItems() {
    ...
    this.adminService.getJobTitles({
  ...
  openEditModal(item: any) {
    ...
    this.formData.set({ title: item.title });
    this.showModal.set(true);
  }
  ...
  save() {
    ...
    if (!title.trim()) {
      this.message.set({ type: 'error', text: 'Title is required.' });
      return;
    }
    this.saving.set(true);

    const operation = this.editingItem()
      ? this.adminService.updateJobTitle(this.editingItem()._id, this.formData())
      : this.adminService.createJobTitle(this.formData());
    ...
    operation.subscribe({
      next: () => {
        ...
        this.closeModal();
        this.loadItems();
        this.message.set({ type: 'success', text: `Job title ${this.editingItem() ? 'updated' : 'created'}!` });
  ...
  toggleStatus(item: any) {
    this.adminService.toggleJobTitleStatus(item._id).subscribe({
      next: (updated) => {
  ...
  deleteItem(item: any) {
    ...
    this.adminService.deleteJobTitle(item._id).subscribe({
      next: () => {
        this.loadItems();
